...
const FName AProjectSteelPawn::LookUpBinding("LookUp");
const FName AProjectSteelPawn::LookRightBinding("LookRight");
const FName AProjectSteelPawn::EngineAudioRPM("RPM");

#define LOCTEXT_NAMESPACE "VehiclePawn"

AProjectSteelPawn::AProjectSteelPawn(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	...
	// Wheels/Tyres
	// Setup the wheels
	Vehicle4W->WheelSetups[0].WheelClass = UProjectSteelWheelFront::StaticClass();
	Vehicle4W->WheelSetups[0].BoneName = FName("PhysWheel_FL");
	Vehicle4W->WheelSetups[0].AdditionalOffset = FVector(0.f, -8.f, 0.f);

	Vehicle4W->WheelSetups[1].WheelClass = UProjectSteelWheelFront::StaticClass();
	Vehicle4W->WheelSetups[1].BoneName = FName("PhysWheel_FR");
	Vehicle4W->WheelSetups[1].AdditionalOffset = FVector(0.f, 8.f, 0.f);

	Vehicle4W->WheelSetups[2].WheelClass = UProjectSteelWheelRear::StaticClass();
	Vehicle4W->WheelSetups[2].BoneName = FName("PhysWheel_BL");
	Vehicle4W->WheelSetups[2].AdditionalOffset = FVector(0.f, -8.f, 0.f);

	Vehicle4W->WheelSetups[3].WheelClass = UProjectSteelWheelRear::StaticClass();
...
void AProjectSteelPawn::SetupPlayerInputComponent(class UInputComponent* InputComponent)
{
	// set up gameplay key bindings
	check(InputComponent);

	InputComponent->BindAxis("MoveForward", this, &AProjectSteelPawn::MoveForward);
	InputComponent->BindAxis("MoveRight", this, &AProjectSteelPawn::MoveRight);
	InputComponent->BindAxis(LookUpBinding);
	InputComponent->BindAxis(LookRightBinding);

	InputComponent->BindAction("Handbrake", IE_Pressed, this, &AProjectSteelPawn::OnHandbrakePressed);
	InputComponent->BindAction("Handbrake", IE_Released, this, &AProjectSteelPawn::OnHandbrakeReleased);
	InputComponent->BindAction("SwitchCamera", IE_Pressed, this, &AProjectSteelPawn::OnToggleCamera);

	InputComponent->BindAction("ResetVR", IE_Pressed, this, &AProjectSteelPawn::OnResetVR); 
}

void AProjectSteelPawn::MoveForward(float Val)
{
	GetVehicleMovementComponent()->SetThrottleInput(Val);

}

void AProjectSteelPawn::MoveRight(float Val)
{
	GetVehicleMovementComponent()->SetSteeringInput(Val);
}

void AProjectSteelPawn::OnHandbrakePressed()
{
	GetVehicleMovementComponent()->SetHandbrakeInput(true);
}

void AProjectSteelPawn::OnHandbrakeReleased()
{
	GetVehicleMovementComponent()->SetHandbrakeInput(false);
}

void AProjectSteelPawn::OnToggleCamera()
{
	EnableIncarView(!bInCarCameraActive);
}

void AProjectSteelPawn::EnableIncarView(const bool bState)
{
...
void AProjectSteelPawn::Tick(float Delta)
{
...
void AProjectSteelPawn::BeginPlay()
{
...
void AProjectSteelPawn::OnResetVR()
{
#ifdef HMD_INTGERATION
...
void AProjectSteelPawn::UpdateHUDStrings()
{
...
void AProjectSteelPawn::SetupInCarHUD()
{
...
void AProjectSteelPawn::UpdatePhysicsMaterial()
{
